...
The document-shaped boxes around the model are best read as the kinds of structured artifacts or views different contributors work with.
Examples include:
...
The model is not designed for one generic user.
It is designed so different roles contribute to and consume different parts of the shared structure.
...
It says the shared model is not populated by central modelling alone.
It is built through ongoing role-based participation.
...
The diagram suggests many role-specific artifacts.
That is useful, but only if those artifacts are treated as **views over canonical semantics**, not as isolated mini-models with their own incompatible meanings.
...
When a person updates a catalog, framework, performance view, or planning artifact, the system should not just store a document.
It should create traceable semantic structure such as:
...
The roles shown in the diagram should not just be seen as access groups.
They are also likely to contribute different kinds of semantics:
...
The diagram strongly implies projection, but it does not say it clearly enough.
...
The collaboration pattern is shown structurally, but not temporally.
A mature version needs to show that each contribution can have:
...
The diagram should make that difference much more explicit.
Right now they are all shown as roughly equivalent document-like inputs.

### 4. Social and relational semantics of collaboration itself

The page is called collaboration, but the collaboration relations are not yet modelled deeply enough.
A richer version would show that collaboration is also grounded in:
